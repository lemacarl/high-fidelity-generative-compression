...
if __name__ == '__main__':
    ...
    arch_args.add_argument('--quantize', action='store_true', help="Enable Quantization Aware Training (QAT).")
    ...
    if args.warmstart is True:
        ...
        if getattr(args, 'quantize', False):
            model.train()
            # Optimizers are already set up inside load_model but we recreate them because parameters changed in load_model
    ...
    logger.info('QUANTIZE: {}'.format(args.quantize))
