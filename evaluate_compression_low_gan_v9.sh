#!/usr/bin/env bash
# evaluate_compression_low_gan_v9.sh
#
...
# models for the tflite_low_gan_v9 checkpoint, for test_image_N (N=1..10) from
# assets/coffee/paper/.
#
# Unlike evaluate_reconstruction_low_gan_v9.sh (GPU, restores the TF
...
#
# THIS SCRIPT IS THE v9 VERDICT.
#   v9 exists to fix a rate bug, so its coded rate is the whole point. The
#   rate-loss clip was raised from -10 to -20 nats (14.43 -> 28.85 bits per
#   symbol) so the trainer can finally see, and get gradient from, the symbols
#   the ANS coder actually charges for: 18.48 bits for a |residual|=1 at
#   sigma=0.11, 17.48 for a tail escape. Under the old clip those were priced
#   flat at 14.43 with zero gradient.
#
#   Measured coded rates on this exact image set, each at its own best
#   --scales_min:
#     low_v2      0.1565 bpp   (best at --scales_min 0.11)
#     low_gan_v7  0.1556 bpp   (best at --scales_min 0.11)
#     low_gan_v8  0.2162 bpp   (best at --scales_min 0.20; 0.2915 at default)
#
#   v9 wants to land near 0.156. Anything near 0.216 means the clip was not
#   the binding constraint.
#
# RUN THE --scales_min SWEEP TOO — this script uses the 0.11 default.
#   v8's optimum was 0.20, not the default, and the difference was 26% of its
#   rate. v9's optimum is unknown until measured; do not assume it inherits
#   either v8's or the default. After this script:
#
#     for SM in 0.11 0.15 0.20 0.25 0.30; do
#       T=0
#       for N in $(seq 1 10); do
#         python -m tflite.inference.compress --compress \
#             -i assets/coffee/paper/test_image_${N}.jpg -o /tmp/sw.hfc \
#             --models_dir tflite_models_low_gan_v9 \
#             --density_weights experiments/tflite_low_gan_v9/density_weights.npz \
#             --scales_min $SM >/dev/null 2>&1
#         T=$(awk "BEGIN{print $T + 8*$(stat -c%s /tmp/sw.hfc)/65536}")
#       done
#       echo "scales_min=$SM  mean coded bpp = $(awk "BEGIN{printf \"%.4f\", $T/10}")"
#     done
#
#   If v9's optimum has moved back to 0.11 that is itself evidence the fix
#   worked: it would mean sigma has left the MIN_SCALE floor and the coder no
#   longer needs a widened table to compensate. Confirm directly with
#   --stats on one image and read the sigma p50 — it was 0.110 for both v7
#   and v8.
#
# Two size columns, because they answer different questions:
#   bpp                 bits per coded pixel — the real rate, and the number to
#                       compare against v7's 0.1556 or against the 0.16 target
#   ratio_vs_input_png  input bytes / .hfc bytes. Inflated, and not a
...
#
# NOTE: tflite_low_gan_v9 must be exported to TFLite first:
#   python -m tflite.conversion.export_tflite \
#       --checkpoint experiments/tflite_low_gan_v9/final-300000 \
#       --out_dir tflite_models_low_gan_v9/
#
# Usage:
#   bash evaluate_compression_low_gan_v9.sh
#   bash evaluate_compression_low_gan_v9.sh --csv my_results.csv
...
#   --checkpoint experiments/tflite_low_gan_v9/final-300000 --out_dir tflite_models_low_gan_v9/`)
MODELS_DIR="tflite_models_low_gan_v9"
DENSITY_WEIGHTS="experiments/tflite_low_gan_v9/density_weights.npz"
...
if [[ ! -d "$MODELS_DIR" ]]; then
    ...
    echo "  python -m tflite.conversion.export_tflite --checkpoint experiments/tflite_low_gan_v9/final-300000 --out_dir $MODELS_DIR"
