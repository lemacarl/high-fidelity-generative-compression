#!/usr/bin/env bash
# evaluate_compression_low_v2.sh
#
...
# models for the tflite_low_v2 checkpoint, for test-image-N (N=1..20) from
# assets/coffee/test-new/.
#
# Usage:
#   bash evaluate_compression_low_v2.sh
#   bash evaluate_compression_low_v2.sh --csv my_results.csv   (override output path)
...
#   --checkpoint experiments/tflite_low_v2/final-2000000 --out_dir tflite_models_v2_lo/`)
...
if [[ ! -d "$MODELS_DIR" ]]; then
    ...
    echo "  python -m tflite.conversion.export_tflite --checkpoint experiments/tflite_low_v2/final-2000000 --out_dir $MODELS_DIR"
