#!/usr/bin/env bash
# evaluate_compression_med_gan.sh
#
...
# models for the med_gan checkpoint, for test-image-N (N=1..20) from
# assets/coffee/test-new/.
#
# Unlike evaluate_reconstruction_med_gan.sh (GPU, restores the TF checkpoint
...
#                       compare against other models or against the 0.30 target
#                       for the med regime
...
#
# EXPORT FIRST — tflite_models_med_gan/ does not exist yet:
#   python -m tflite.conversion.export_tflite \\
#       --checkpoint experiments/tflite_med_gan/final-200000 \\
#       --out_dir tflite_models_med_gan/
#
#   Without it every row lands as SKIPPED_NO_MODEL_DIR rather than failing
#   silently. density_weights.npz lives beside the checkpoint, not in the
#   models dir, because the export step does not copy it.
#
#
# Usage:
#   bash evaluate_compression_med_gan.sh
#   bash evaluate_compression_med_gan.sh --csv my_results.csv   (override output path)
...
# ── Configurable paths ──────────────────────────────────────────────────────
VARIANT="med_gan"
MODELS_DIR="tflite_models_med_gan"
DENSITY_WEIGHTS="experiments/tflite_med_gan/density_weights.npz"
...
if [[ ! -d "$MODELS_DIR" ]]; then
    ...
    echo "  python -m tflite.conversion.export_tflite --checkpoint experiments/tflite_med_gan/final-200000 --out_dir $MODELS_DIR"
...
for N in $(seq 1 "$NUM_IMAGES"); do
    ...
    OUTPUT_FILE="${OUTPUT_DIR}/test_image-${N}.hfc"
